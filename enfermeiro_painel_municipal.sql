-- View: pmaq."ENFERMEIRO_PAINEL_MUNICIPAL"
DROP VIEW if exists pmaq."ENFERMEIRO_PAINEL_MUNICIPAL";

CREATE OR REPLACE VIEW pmaq."ENFERMEIRO_PAINEL_MUNICIPAL" AS 
 SELECT tb_dim_municipio_1.no_municipio,
  ...
  WHERE tb_dim_cbo.no_cbo::text = 'ENFERMEIRO DA ESTRATÉGIA DE SAÚDE DA FAMÍLIA'::text AND tt.nu_ano::double precision = date_part('year'::text, 'now'::text::date) AND tt.nu_mes::double precision = date_part('month'::text, 'now'::text::date)
  GROUP BY tb_dim_municipio_1.no_municipio, tb_dim_cbo.no_cbo, tt.nu_ano, tt.nu_mes;

ALTER TABLE pmaq."ENFERMEIRO_PAINEL_MUNICIPAL"
  OWNER TO postgres;
GRANT ALL ON TABLE pmaq."ENFERMEIRO_PAINEL_MUNICIPAL" TO postgres;
GRANT ALL ON TABLE pmaq."ENFERMEIRO_PAINEL_MUNICIPAL" TO pmaq;
